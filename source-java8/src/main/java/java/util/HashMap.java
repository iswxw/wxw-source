...
public class HashMap<K, V> extends AbstractMap<K, V>
     ...
     */
    static final int DEFAULT_INITIAL_CAPACITY = 1 << 4; // aka 16  //默认的初始容量为16，必须是2的幂次
    ...
    static final int MIN_TREEIFY_CAPACITY = 64; // 树化阈值的最小容量是64，否则进行扩容（可以扩容四次避免树化和扩容之间的冲突）,而不进行树化
     ...
     */
    transient int size; // HashMap实际存储的Node数量，注意这个数量不等于table的长度，甚至可能大于它，因为在table的每个节点上是一个链表（或RBT）结构，可能不止有一个Node元素存在
     ...
     */
    transient int modCount;//用于迭代过程中的Fail-Fast机制，其存在的意义在于保证发生了线程安全问题时，能及时的发现（操作前备份的count和当前modCount不相等）并抛出异常终止操作
    ...
    int threshold; // threshold = capacity * load factor

    /**
     * The load factor for the hash table.
     * 负载因子 计算出当前table长度下的扩容阈值：threshold = loadFactor * table.length。
     *
     * @serial
     */
    ...
    public V put(K key, V value) {
        /**
         *  onlyIfAbsent = false 替换已经存在的值
         *  evict[驱逐，赶出] = true  代表是在创建　hashMap 后才调用这个函数
         */
     ...
     * @param evict        if false, the table is in creation mode.[false表示hash表处于创建模式]  // evict参数用于LinkedHashMap中的尾部操作，这里没有实际意义。
     * @return previous value[以前的值], or null if none
     */
    ...
    // 如果已经存在 value就不执行put操作，直接返回旧值，否则进行put操作【其实就是onlyIfAbsent = true】
    @Override
